...
- **Transport**: `MyPluginRecordFetcher` implements `RecordFetcher<String>` from ktestify-core. It returns `List<ConsumedRecord<String>>`, the only type crossing layer boundaries.
- **Orchestration**: `MyPluginConsumer` wires fetch to match to result. It uses `RecordMatcherFactory.forRaw()` to select the correct matcher.
- **Assertion**: all standard `RecordMatcher` implementations from ktestify-core are reused as-is. No matcher code is needed in your plugin.
...
If this succeeds, you have a valid plugin skeleton. The generated code compiles out of the box, and all TODO stubs are syntactically valid.
