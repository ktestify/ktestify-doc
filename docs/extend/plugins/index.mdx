...
- Project setup using the Maven archetype (`ktestify-plugin-archetype`)
- Archetype parameters and project generation
...
Start with the Maven archetype, which generates a complete, compilable project in seconds:
...
mvn archetype:generate \
  ...
  -DartifactId=ktestify-plugin-myplugin \
  -Dversion=1.0-SNAPSHOT \
  -Dpackage=io.github.ktestify.myplugin \
  -DpluginShortName=myplugin \
  -DpluginKebabId=my-plugin \
  -DpluginPascalName=MyPlugin \
  -DinteractiveMode=false
```

Then follow the [Creating a Plugin](create-plugin) guide to implement your transport logic.
...
| **Notifications** | 🚧 In progress | Send suite summary cards to Teams, Slack, or any HTTP webhook |
...
7. **Use the archetype**, start from `ktestify-plugin-archetype` to get the correct structure and build setup
...
- [Build your first plugin](create-plugin) with the Maven archetype
- Explore the [Azure Blob reference](azureblob)
- Send results with the [Notifications reference](notifications)
- Check out the [plugin archetype](https://github.com/ktestify/ktestify-plugin-archetype)
