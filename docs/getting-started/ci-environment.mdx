...
regression-tests:
  stage: test
  image: ghcr.io/ktestify/ktestify-cucumber:latest
  variables:
    KTESTIFY_CONFIG_FILE: $CI_PROJECT_DIR/.config/ktestify.conf
  rules:
  ...
  artifacts:
    when: always
    paths:
      - $CI_PROJECT_DIR/workspace/reports
```
...
- [Configuration →](configuration), `topic-namespace` and all environment variable overrides
